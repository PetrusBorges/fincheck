    ...
    <div>
      <div className="relative">
        ...
        <RdxSelect.Root value={value} onValueChange={handleSelect}>
          <RdxSelect.Trigger
            className={cn(
              "w-full bg-white rounded-lg border border-gray-400 px-3 h-[52px] text-gray-800 focus:border-teal-500 transition-all outline-none appearance-none text-left relative pt-4",
              ...
              <RdxSelect.Viewport className="p-2">
                {options.map((option) => (
                  <RdxSelect.Item
                    key={option.value}
                    value={option.value}
                    className='cursor-pointer p-2 text-gray-800 text-sm data-[state=checked]:font-bold outline-none data-[highlighted]:bg-gray-100 rounded-lg transition-colors'
                  >
